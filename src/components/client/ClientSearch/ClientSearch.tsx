...
import type { Placement } from "@vkontakte/vkui/dist/cssm/lib/floating/types/common";
...
const SearchComponentChecked = (
	<Icon16Done role="presentation" fill="currentColor" width={16} height={16} />
);
...
export const ClientSearch: FC = () => {
	...
	const handleSelect = useStableHandler(
	...
	if (!open) {
		render = null;
	} else if ((search.length > 0 && data.length === 0) || error) {
		...
		render = data.map((option) => (
			<SimpleCell
				...
				after={selected === option.package.name ? SearchComponentChecked : null}
			>
	...
	const handlePlacement = useHandler((update: Placement) => {
	...
	return (
		<label ref={targetRef} className={styles.label}>
			<Input
				...
				onFocus={handleFocus}
				onBlur={handleBlur}
				after={isValidating ? SearchComponentSpinner : SearchComponentIcon}
				id="search"
				name="search"
				type="search"
				inputMode="search"
