...
export const internal = (req: NextRequest) =>
	req.headers.has(InternalHeader.NAME);

const ok = (status: number) => status === 200;

const fresh = (headers: HeadersInit) =>
	Object.assign(headers, {
		"Cache-Control": "no-store",
		"CDN-Cache-Control": "no-store",
	});

const cached = (headers: HeadersInit) =>
	Object.assign(headers, {
...
export const respondJSON = (status: number, json: Record<string, unknown>) =>
	respond(status, JSON.stringify(json), ContentType.JSON);

export const respondSVG = (status: number, svg: BodyInit) =>
	respond(status, svg, ContentType.SVG);

export const respondJPEG = (status: number, jpeg: BodyInit) =>
	respond(status, jpeg, ContentType.JPEG);

export const respondInternal = (status: number, body: BodyInit) =>
	respond(status, body, ContentType.INTERNAL);

export const respondNothing = (status: number) =>
	respond(status, null, ContentType.UNKNOWN);
